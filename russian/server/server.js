...
// Simple word bank by categories (English UI)
const WORDS = {
	"Food": [
		{ ru: "яблоко", hint: "Apple" },
		{ ru: "хлеб", hint: "Bread" },
		{ ru: "сыр", hint: "Cheese" },
	],
	"Animals": [
...
function startRound() {
	const chosen = pickWord(state.category === 'All' ? null : state.category);
	...
	ws.on('message', (raw) => {
		try {
			...
			if (msg.type === 'join') {
				...
				const category = ['Food', 'Animals', 'Transport'].includes(msg.category) ? msg.category : 'All';
